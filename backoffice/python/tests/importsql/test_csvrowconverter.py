...
class RowConverterTest(unittest.TestCase):
    def test_convert(self):
        ...
        row_input = {
            ...
            'explanation.definition': 'pin yin'}
        foreign_keys = [ForeignKey(foreign_key_column)]
        rowConverter = RowConverter(row_input, foreign_keys)
        converted_statements = rowConverter.convert()
        self.assertEqual(2, len(converted_statements))
        expected_statement1 = """MERGE INTO lexical_item
USING (SELECT 'en' AS language,'value' AS value) AS u
    ON lexical_item.language = u.language AND lexical_item.value = u.value
WHEN MATCHED THEN
